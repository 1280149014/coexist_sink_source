...
static bool process_read_multi_rsp(tGATT_SR_CMD* p_cmd, tGATT_STATUS status,
  ...
  LOG(ERROR) << StringPrintf("%s status=%d mtu=%d", __func__, status, mtu);
  ...
  if (status == GATT_SUCCESS) {
    LOG(ERROR) << "Multi read count=" << fixed_queue_length(p_cmd->multi_rsp_q)
    ...
    if (fixed_queue_length(p_cmd->multi_rsp_q) ==
      ...
      *p++ = GATT_RSP_READ_MULTI;
      ...
      if (p_buf->len == 0) {
        ...
        p_cmd->status = GATT_NOT_FOUND;
        osi_free(p_buf);
        LOG(ERROR) << __func__ << "osi_free(p_buf)";
...
tGATT_STATUS gatt_sr_process_app_rsp(tGATT_TCB& tcb, tGATT_IF gatt_if,
  ...
  LOG(ERROR) << __func__ << " gatt_if=" << +gatt_if;
  ...
  LOG(ERROR) << __func__ << " ret_code=" << +ret_code;
...
#if (GATT_CONFORMANCE_TESTING == TRUE)
  if (gatt_cb.enable_err_rsp && gatt_cb.req_op_code == op_code) {
    LOG(ERROR) << "Conformance tst: forced err rspvofr ReadMultiple: error status="
            << +gatt_cb.err_status;
  ...
  while (ll >= 2 &&
    ...
    if (it != gatt_cb.srv_list_info->end()) {
      ...
      err = gatts_read_attr_perm_check(it->p_db, false, handle, sec_flag,
                                       key_size);
      if (err != GATT_SUCCESS) {
        LOG(ERROR) << StringPrintf("read permission denied : 0x%02x", err);
        break;
      }
    } else {
      /* invalid handle */
...
static tGATT_STATUS gatt_build_primary_service_rsp(
  ...
  for (tGATT_SRV_LIST_ELEM& el : *gatt_cb.srv_list_info) {
    ...
    if (gatt_cb.last_service_handle &&
      ...
      LOG(ERROR) << "Use 0xFFFF for the last primary attribute";
...
void gatts_process_primary_service_req(tGATT_TCB& tcb, uint8_t op_code,
  ...
  if (uuid != Uuid::From16Bit(GATT_UUID_PRI_SERVICE)) {
    if (op_code == GATT_REQ_READ_BY_GRP_TYPE) {
      ...
      LOG(ERROR) << StringPrintf("unexpected ReadByGrpType Group: %s",
                              uuid.ToString().c_str());
      return;
    }
    ...
    LOG(ERROR) << StringPrintf("unexpected ReadByTypeValue type: %s",
                            uuid.ToString().c_str());
    return;
  }
...
#if (GATT_CONFORMANCE_TESTING == TRUE)
  if (gatt_cb.enable_err_rsp && gatt_cb.req_op_code == op_code) {
    LOG(ERROR) << "Conformance tst: forced err rsp for ReadByType: error status="
            << +gatt_cb.err_status;
  ...
  switch (op_code) {
    ...
    case GATT_SIGN_CMD_WRITE:
      if (op_code == GATT_SIGN_CMD_WRITE) {
        LOG(ERROR) << "Write CMD with data sigining";
...
void gatts_process_attribute_req(tGATT_TCB& tcb, uint8_t op_code, uint16_t len,
  ...
  if (gatt_cb.enable_err_rsp && gatt_cb.req_op_code == op_code) {
    LOG(ERROR) << "Conformance tst: forced err rsp: error status="
            << +gatt_cb.err_status;
...
void gatts_proc_srv_chg_ind_ack(tGATT_TCB tcb) {
  ...
  if (p_buf != NULL) {
    LOG(ERROR) << "NV update set srv chg = false";
...
static bool gatts_proc_ind_ack(tGATT_TCB& tcb, uint16_t ack_handle) {
  ...
  LOG(ERROR) << __func__ << " ack handle=%d" << ack_handle;
