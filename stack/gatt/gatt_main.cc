...
bool gatt_disconnect(tGATT_TCB* p_tcb) {
  ...
  if (ch_state == GATT_CH_CLOSING) {
    LOG(ERROR) << __func__ << " already in closing state";
    return true;
  }
  ...
  if (p_tcb->att_lcid == L2CAP_ATT_CID) {
    ...
    if ((ch_state == GATT_CH_OPEN) || (ch_state == GATT_CH_CFG))
      ...
      LOG(ERROR) << __func__ << " gatt_disconnect channel not opened";
  }
...
bool gatt_update_app_hold_link_status(tGATT_IF gatt_if, tGATT_TCB* p_tcb,
  ...
  if (is_add) {
    ...
    if (ret.second) {
      LOG(ERROR) << "added gatt_if=" << +gatt_if;
    } else {
      LOG(ERROR) << "attempt to add already existing gatt_if=" << +gatt_if;
    }
    return true;
  }
  ...
  if (!holders.erase(gatt_if)) {
    LOG(ERROR) << "attempt to remove nonexisting gatt_if=" << +gatt_if;
    return false;
  }

  LOG(ERROR) << "removed gatt_if=" << +gatt_if;
  return true;
}
...
void gatt_update_app_use_link_flag(tGATT_IF gatt_if, tGATT_TCB* p_tcb,
  ...
  LOG(ERROR) << StringPrintf("%s: is_add=%d chk_link=%d", __func__, is_add,
                          check_acl_link);
  ...
  if (is_add) {
    if (p_tcb->att_lcid == L2CAP_ATT_CID && is_valid_handle) {
      LOG(ERROR) << "disable link idle timer";
    ...
    if (p_tcb->app_hold_link.empty()) {
      ...
      if (p_tcb->att_lcid == L2CAP_ATT_CID && is_valid_handle) {
        ...
        LOG(ERROR) << " start link idle timer = "
...
static void gatt_le_connect_cback(uint16_t chan, const RawAddress& bd_addr,
  ...
  LOG(ERROR) << "GATT   ATT protocol channel with BDA: " << bd_addr << " is "
...
void gatt_init_srv_chg(void) {
  ...
  LOG(ERROR) << __func__;
  if (!gatt_cb.cb_info.p_srv_chg_callback) {
    LOG(ERROR) << __func__ << " callback not registered yet";
    return;
  }
  ...
  LOG(ERROR) << "num_srv_chg_clt_clients=" << +rsp.num_clients;
...
void gatt_proc_srv_chg(void) {
  ...
  while (gatt_find_the_connected_bda(start_idx, bda, &found_idx, &transport)) {
    ...
    if (gatt_is_srv_chg_ind_pending(p_tcb)) {
      ...
      LOG(ERROR) << "discard srv chg - already has one in the queue";
    }
    ...
    if (send_indication &&
      ...
      if (interop_match_name(INTEROP_GATTC_NO_SERVICE_CHANGED_IND,
                             remote_name)) {
        LOG(ERROR) << "discard srv chg - interop matched " << remote_name;
        send_indication = false;
      }
    }
...
void gatt_set_ch_state(tGATT_TCB* p_tcb, tGATT_CH_STATE ch_state) {
  ...
  LOG(ERROR) << __func__ << ": old=" << +p_tcb->ch_state
          << " new=" << loghex(ch_state);
...
tGATT_CH_STATE gatt_get_ch_state(tGATT_TCB* p_tcb) {
  ...
  LOG(ERROR) << "gatt_get_ch_state: ch_state=" << +p_tcb->ch_state;
