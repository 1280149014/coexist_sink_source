...
static void srvc_eng_s_request_cback(uint16_t conn_id, uint32_t trans_id,
  ...
  LOG(ERROR) << StringPrintf("srvc_eng_s_request_cback : recv type (0x%02x)",
                          type);
  ...
  switch (type) {
    ...
    case GATTS_REQ_TYPE_WRITE_EXEC:
      LOG(ERROR) << "Ignore GATT_REQ_EXEC_WRITE/WRITE_CMD";
      break;
...
tGATT_STATUS srvc_eng_init(void) {
  ...
  if (srvc_eng_cb.enabled) {
    ...
    LOG(ERROR) << "Srvc_Init:  gatt_if=" << +srvc_eng_cb.gatt_if;
