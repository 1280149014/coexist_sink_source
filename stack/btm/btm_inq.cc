...
tBTM_STATUS BTM_ReadRemoteDeviceName(const RawAddress& remote_bda,
  ...
  LOG(ERROR) << __func__ << ": bd addr " << remote_bda;
...
tBTM_INQ_INFO* BTM_InqDbRead(const RawAddress& p_bda) {
  LOG(ERROR) << __func__ << ": bd addr " << p_bda;
...
static tBTM_STATUS btm_set_inq_event_filter(uint8_t filter_cond_type,
  ...
  BTM_TRACE_DEBUG(
      ...
      filter_cond_type);
  LOG(ERROR) << "condition " << p_filt_cond->bdaddr_cond;
#endif
...
void btm_process_remote_name(const RawAddress* bda, BD_NAME bdn,
  ...
  LOG(ERROR) << "Inquire BDA " << p_inq->remname_bda;
