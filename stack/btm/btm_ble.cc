...
bool BTM_SecAddBleKey(const RawAddress& bd_addr, tBTM_LE_KEY_VALUE* p_le_key,
  ...
  LOG(ERROR) << __func__ << " BDA: " << bd_addr << ", Type: " << key_type;
...
void read_phy_cb(
  ...
  LOG(ERROR) << __func__ << " Received read_phy_cb";
...
bool btm_get_local_div(const RawAddress& bd_addr, uint16_t* p_div) {
  tBTM_SEC_DEV_REC* p_dev_rec;
  bool status = false;
  LOG(ERROR) << __func__ << " bd_addr: " << bd_addr;
...
void btm_sec_save_le_key(const RawAddress& bd_addr, tBTM_LE_KEY_TYPE key_type,
  ...
  if ((p_rec = btm_find_dev(bd_addr)) != NULL &&
    ...
    LOG(ERROR) << "BLE key type 0x" << loghex(key_type)
...
void btm_ble_connected(const RawAddress& bda, uint16_t handle, uint8_t enc_mode,
  ...
  */
  if (p_dev_rec) {
    LOG(ERROR) << __func__ << " Security Manager: handle:" << handle
    ...
    switch (event) {
      ...
      case SMP_COMPLT_EVT:
        ...
        if (event == SMP_COMPLT_EVT) {
          ...
          LOG(ERROR) << "btm_cb.pairing_bda: " << btm_cb.pairing_bda;
