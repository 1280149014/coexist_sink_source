...
void btm_acl_created(const RawAddress& bda, DEV_CLASS dc, BD_NAME bdn,
  ...
  if (p != (tACL_CONN*)NULL) {
    ...
    LOG(ERROR) << "Duplicate btm_acl_created: RemBdAddr: " << bda;
...
tBTM_STATUS BTM_SwitchRole(const RawAddress& remote_bd_addr, uint8_t new_role,
  ...
  if (btm_cb.devcb.p_switch_role_cb && p_cb) {
    LOG(ERROR) << "Role switch on other device is in progress "
...
bool BTM_IsAclConnectionUp(const RawAddress& remote_bda,
  ...
  LOG(ERROR) << __func__ << " RemBdAddr: " << remote_bda;
...
void btm_acl_paging(BT_HDR* p, const RawAddress& bda) {
  ...
  LOG(ERROR) << __func__ << ":" << btm_cb.discing << " , paging:" << btm_cb.paging
          << " BDA: " << bda;
  ...
  if (btm_cb.discing) {
    ...
    if (!BTM_ACL_IS_CONNECTED(bda)) {
      LOG(ERROR) << "connecting_bda: " << btm_cb.connecting_bda;
...
bool btm_acl_notif_conn_collision(const RawAddress& bda) {
  ...
  if (btm_cb.p_bl_changed_cb) {
    LOG(ERROR) << __func__ << " RemBdAddr: " << bda;
