      ...
      switch (db_attr.uuid) {
        case GATT_UUID_GAP_DEVICE_NAME:
          ...
          if (offset > p_value->len)
            ...
            LOG(ERROR) << "GATT_UUID_GAP_DEVICE_NAME len=" << +p_value->len;
          }
          break;
...
void server_attr_request_cback(uint16_t conn_id, uint32_t trans_id,
  ...
  LOG(ERROR) << StringPrintf("%s: recv type (0x%02x)", __func__, type);
  ...
  switch (type) {
    ...
    case GATTS_REQ_TYPE_WRITE_EXEC:
      ignore = true;
      LOG(ERROR) << "Ignore GATTS_REQ_TYPE_WRITE_EXEC";
      break;

    case GATTS_REQ_TYPE_MTU:
      LOG(ERROR) << "Get MTU exchange new mtu size: " << +p_data->mtu;
      ignore = true;
      break;

    default:
      LOG(ERROR) << StringPrintf("Unknown/unexpected LE GAP ATT request: 0x%02x",
                               type);
      break;
  }
...
void cl_op_cmpl(tGAP_CLCB& clcb, bool status, uint16_t len, uint8_t* p_name) {
  ...
  LOG(ERROR) << StringPrintf("%s: status: %d", __func__, status);

  clcb.cl_op_uuid = 0;
  clcb.p_cback = NULL;

  if (p_cback && op) {
    LOG(ERROR) << __func__ << ": calling";
...
bool accept_client_operation(const RawAddress& peer_bda, uint16_t uuid,
  ...
  LOG(ERROR) << __func__ << ": BDA: " << peer_bda
...
void gap_attr_db_init(void) {
  ...
  LOG(ERROR) << __func__ << ": service_handle = " << +service_handle;
...
void GAP_BleAttrDBUpdate(uint16_t attr_uuid, tGAP_BLE_ATTR_VALUE* p_value) {
  LOG(ERROR) << StringPrintf("%s: attr_uuid=0x%04x", __func__, attr_uuid);

  for (tGAP_ATTR& db_attr : gatt_attr) {
    if (db_attr.uuid == attr_uuid) {
      LOG(ERROR) << StringPrintf("Found attr_uuid=0x%04x", attr_uuid);
