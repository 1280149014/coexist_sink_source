...
/* register with no authorization; let AVDTP use authorization instead */
    ...
    if ((btif_av_src_sink_coexist_enabled() &&
      ...
      if (profile_initialized == UUID_SERVCLASS_AUDIO_SOURCE) {
        APPL_TRACE_WARNING("UUID_SERVCLASS_AUDIO_SOURCE 0000");
...
#if (BTA_AV_SINK_INCLUDED == TRUE)
        APPL_TRACE_WARNING("UUID_SERVCLASS_AUDIO_SINK 1111");
        ...
        if (btif_av_src_sink_coexist_enabled()) {
          if (profile_initialized == UUID_SERVCLASS_AUDIO_SOURCE) {
            ...
            bta_ar_reg_avrc_for_src_sink_coexist(
                ...
                (bta_av_cb.features & BTA_AV_FEAT_BROWSE), AVRC_REV_1_6); // todo ...
        } else {
          if (profile_initialized == UUID_SERVCLASS_AUDIO_SOURCE &&
  ...
  if (id) {
    ...
    for (i = 0; i < BTA_AV_NUM_STRS; i++) {
      ...
      if (p_scb && p_scb->co_started && (!p_scb->sco_suspend)) {
        LOG(ERROR) << __func__ << ": suspending scb:" << i;
    ...
    for (i = 0; i < BTA_AV_NUM_STRS; i++) {
      ...
      {
        LOG(ERROR) << __func__ << ": starting scb:" << i;
