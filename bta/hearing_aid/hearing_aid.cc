...
class HearingDevices {
 public:
  ...
  void StartRssiLog() {
    ...
    for (auto& d : devices) {
      LOG(ERROR) << __func__ << ": device=" << d.address << ", read_rssi_count=" << d.read_rssi_count;
 ...
 public:
  ...
  uint16_t UpdateBleConnParams(const RawAddress& address) {
    ...
    if (overwrite_min_ce_len != 0) {
      LOG(ERROR) << __func__ << ": min_ce_len=" << min_ce_len
  ...
  void AddFromStorage(const HearingDevice& dev_info, uint16_t is_white_listed) {
    LOG(ERROR) << __func__ << " " << dev_info.address
  ...
  void OnGattConnected(tGATT_STATUS status, uint16_t conn_id,
    ...
    LOG(ERROR) << __func__ << ": address=" << address << ", conn_id=" << conn_id;
  ...
  void OnReadRssiComplete(const RawAddress& address, int8_t rssi_value) {
    ...
    LOG(ERROR) << __func__ << ": device=" << address << ", rssi=" << (int)rssi_value;
    ...
    for (const gatt::Characteristic& charac : service->characteristics) {
      if (charac.uuid == READ_ONLY_PROPERTIES_UUID) {
        if (!btif_storage_get_hearing_aid_prop(
          ...
          LOG(ERROR) << "Reading read only properties "
  ...
  void OnReadOnlyPropertiesRead(uint16_t conn_id, tGATT_STATUS status,
    ...
    LOG(ERROR) << __func__ << " " << base::HexEncode(value, len);
    ...
    LOG(ERROR) << __func__ << " capabilities: " << (side ? "right" : "left")
    ...
    LOG(ERROR) << __func__ << " hiSyncId: " << loghex(hearingDevice->hi_sync_id);
    uint8_t feature_map;
    ...
    LOG(ERROR) << __func__ << " preparation delay: "
            << loghex(hearingDevice->preparation_delay);

    uint16_t codecs;
    STREAM_TO_UINT16(codecs, p);
    hearingDevice->codecs = codecs;
    LOG(ERROR) << __func__ << " supported codecs: " << loghex(codecs);
    if (codecs & (1 << CODEC_G722_16KHZ)) LOG(ERROR) << "\tG722@16kHz";
    if (codecs & (1 << CODEC_G722_24KHZ)) LOG(ERROR) << "\tG722@24kHz";
  ...
  void OnPsmRead(uint16_t conn_id, tGATT_STATUS status, uint16_t handle,
    ...
    HearingDevice* hearingDevice = hearingDevices.FindByConnId(conn_id);
    if (!hearingDevice) {
      LOG(ERROR) << "Skipping unknown read event, conn_id=" << loghex(conn_id);
      return;
    }
  ...
  void StartSendingAudio(const HearingDevice& hearingDevice) {
    LOG(ERROR) << __func__ << ": device=" << hearingDevice.address;
  ...
  void SendEnableServiceChangedInd(HearingDevice* device) {
    LOG(ERROR) << __func__ << " Enable " << device->address
            << "service changed ind.";
    std::vector<uint8_t> value(2);
    uint8_t* ptr = value.data();
  ...
  void OnAudioDataReady(const std::vector<uint8_t>& data) {
    ...
    if (left) {
      ...
      if (packets_to_flush) {
        LOG(ERROR) << left->address << " skipping " << packets_to_flush
                << " packets";
    ...
    if (right) {
      ...
      uint16_t packets_to_flush = L2CA_FlushChannel(cid, L2CAP_FLUSH_CHANS_GET);
      if (packets_to_flush) {
        LOG(ERROR) << right->address << " skipping " << packets_to_flush
                << " packets";
  ...
  void SendAudio(uint8_t* encoded_data, uint16_t packet_size,
    ...
    *p = seq_counter;
    ...
    LOG(ERROR) << hearingAid->address << " : " << base::HexEncode(p, packet_size);
  ...
  void GapCallback(uint16_t gap_handle, uint16_t event, tGAP_CB_DATA* data) {
    ...
    switch (event) {
      ...
      case GAP_EVT_CONN_DATA_AVAIL: {
        LOG(ERROR) << "GAP_EVT_CONN_DATA_AVAIL";
        ...
        LOG(ERROR) << "stats from the hearing aid:";
        for (size_t i = 0; i + 4 <= buffer.size(); i += 4) {
          ...
          LOG(ERROR) << "event_counter=" << event_counter
      ...
      case GAP_EVT_TX_EMPTY:
        LOG(ERROR) << "GAP_EVT_TX_EMPTY";
        break;
      case GAP_EVT_CONN_CONGESTED:
        LOG(ERROR) << "GAP_EVT_CONN_CONGESTED";
      ...
      case GAP_EVT_CONN_UNCONGESTED:
        LOG(ERROR) << "GAP_EVT_CONN_UNCONGESTED";
        break;

      case GAP_EVT_LE_COC_CREDITS: {
        auto& tmp = data->coc_credits;
        LOG(ERROR) << "GAP_EVT_LE_COC_CREDITS, for device: "
  ...
  void OnGattDisconnected(tGATT_STATUS status, uint16_t conn_id,
    ...
    LOG(ERROR) << __func__ << ": conn_id=" << loghex(conn_id)
  ...
  void check_and_do_rssi_read(HearingDevice* device) {
    if (device->read_rssi_count > 0) {
      ...
      if (device->num_intervals_since_last_rssi_read >= PERIOD_TO_READ_RSSI_IN_INTERVALS) {
        ...
        LOG(ERROR) << __func__ << ": device=" << device->address;
...
void hearingaid_gattc_callback(tBTA_GATTC_EVT event, tBTA_GATTC* p_data) {
  LOG(ERROR) << __func__ << " event = " << +event;
