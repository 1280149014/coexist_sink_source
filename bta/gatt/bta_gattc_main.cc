...
bool bta_gattc_sm_execute(tBTA_GATTC_CLCB* p_clcb, uint16_t event,
...
#if (BTA_GATT_DEBUG == TRUE)
  LOG(ERROR) << StringPrintf("%s: State 0x%02x [%s], Event 0x%x[%s]", __func__,
                          in_state, gattc_state_code(in_state), in_event,
                          gattc_evt_code(in_event));
#else
  LOG(ERROR) << StringPrintf("%s: State 0x%02x, Event 0x%x", __func__, in_state,
                          in_event);
#endif
  ...
  if (in_state != p_clcb->state) {
    LOG(ERROR) << StringPrintf("GATTC State Change: [%s] -> [%s] after Event [%s]",
...
bool bta_gattc_hdl_event(BT_HDR* p_msg) {
  ...
  bool rt = true;
#if (BTA_GATT_DEBUG == TRUE)
  LOG(ERROR) << __func__ << ": Event:" << gattc_evt_code(p_msg->event);
#endif
  switch (p_msg->event) {
