...
static void bta_gattc_explore_next_service(uint16_t conn_id,
  ...
  LOG(ERROR) << "Start service discovery";
...
void bta_gattc_start_disc_char_dscp(uint16_t conn_id,
                                    tBTA_GATTC_SERV* p_srvc_cb) {
  LOG(ERROR) << "starting discover characteristics descriptor";
...
descriptor_discovery_done:
  ...
  LOG(ERROR) << "all characteristics explored";
...
#if (BTA_GATT_DEBUG == TRUE)
    LOG(ERROR) << "Found ATT service uuid=" << service_uuid
...
#if (BTA_GATT_DEBUG == TRUE)
    LOG(ERROR) << __func__ << "found service " << service.uuid
