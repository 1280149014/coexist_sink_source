 ...
 public:
  ...
  bool GetPresentationPosition(uint64_t* remote_delay_report_ns,
    ...
    *data_position = data_position_;
    LOG(ERROR) << __func__ << ": delay=" << remote_delay_report_
  ...
  void MetadataChanged(const source_metadata_t& source_metadata) override {
    ...
    LOG(ERROR) << __func__ << ": " << track_count << " track(s) received";
    while (track_count) {
      LOG(ERROR) << __func__ << ": usage=" << tracks->usage
...
void set_remote_delay(uint16_t delay_report) {
  ...
  LOG(ERROR) << __func__ << ": DELAY " << static_cast<float>(delay_report / 10.0)
          << " ms";
  static_cast<A2dpTransport*>(active_hal_interface->GetTransportInstance())
