...
Status BluetoothLeAdvertiserBinderServer::StartMultiAdvertising(
  ...
  auto callback = [=](bluetooth::BLEStatus status) {
    ...
    if (!sp_to_this.get()) {
      LOG(ERROR) << "BluetoothLeAdvertiserBinderServer was deleted";
      return;
    }
    ...
    if (!cb.get()) {
      LOG(ERROR) << "Advertiser was removed before callback: " << advertiser_id;
      return;
    }
...
Status BluetoothLeAdvertiserBinderServer::StopMultiAdvertising(
  ...
  auto callback = [=](bluetooth::BLEStatus status) {
    ...
    auto cb = GetLECallback(advertiser_id);
    if (!cb.get()) {
      LOG(ERROR) << "Advertiser was unregistered - advertiser_id: "
              << advertiser_id;
      return;
    }
