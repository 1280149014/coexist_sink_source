...
bool RemoteCallbackList<T>::Register(const sp<T>& callback) {
  ...
  if (callbacks_.find(binder.get()) != callbacks_.end()) {
    LOG(ERROR) << "Callback list already contains given callback";
    return false;
  }
  ...
  LOG(ERROR) << "Callback successfully registered with list";
...
bool RemoteCallbackList<T>::Unregister(const sp<T>& callback) {
  ...
  if (iter == callbacks_.end()) {
    LOG(ERROR) << "Given callback not registered with this list";
    return false;
  }
...
bool RemoteCallbackList<T>::UnregisterInternal(
  ...
  LOG(ERROR) << "Callback successfully removed from list";
...
void RemoteCallbackList<T>::CallbackDeathRecipient::binderDied(
  ...
  LOG(ERROR) << "Callback from dead process unregistered";
}
