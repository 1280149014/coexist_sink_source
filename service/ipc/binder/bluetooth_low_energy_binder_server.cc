...
Status BluetoothLowEnergyBinderServer::Connect(int client_id,
                                               ...
                                               bool is_direct,
                                               bool* _aidl_return) {
  LOG(ERROR) << __func__ << " client_id: " << client_id << " address: " << address
          << " is_direct: " << is_direct;
...
void BluetoothLowEnergyBinderServer::OnConnectionState(
    ...
    bool connected) {
  LOG(ERROR) << __func__ << " address: " << address << " connected: " << connected;

  int client_id = client->GetInstanceId();
  auto cb = GetLECallback(client->GetInstanceId());
  if (!cb.get()) {
    LOG(ERROR) << "Client was unregistered - client_id: " << client_id;
    return;
  }
...
void BluetoothLowEnergyBinderServer::OnMtuChanged(
    bluetooth::LowEnergyClient* client, int status, const char* address,
    int mtu) {
  LOG(ERROR) << __func__ << " address: " << address << " status: " << status
          << " mtu: " << mtu;
