  ...
  bluetooth::BluetoothInstanceFactory::RegisterCallback cb =
      [weak_ptr_to_this](
        ...
        if (!strong_ptr_to_this.get()) {
          LOG(ERROR) << "InterfaceWithInstancesBase was deleted while instance was"
                  << " being registered";
          return;
        }
...
void InterfaceWithInstancesBase::OnRegisterInstance(
  ...
  if (!callback.get()) {
    LOG(ERROR) << "Callback was removed before the call to \"RegisterInstance\" "
  ...
  LOG(ERROR) << "Registered BluetoothInstance - ID: " << instance_id;
...
void InterfaceWithInstancesBase::OnRemoteCallbackRemoved(const int& key) {
  LOG(ERROR) << __func__ << " instance_id: " << key;
  std::lock_guard<std::mutex> lock(maps_lock_);
