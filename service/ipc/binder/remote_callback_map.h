...
bool RemoteCallbackMap<K, V>::Register(const K& key, const sp<V>& callback,
  ...
  if (map_.find(key) != map_.end()) {
    LOG(ERROR) << "Callback map already contains key";
    return false;
  }
  ...
  LOG(ERROR) << "Callback successfully registered with map";
...
bool RemoteCallbackMap<K, V>::Unregister(const K& key) {
  ...
  if (iter == map_.end()) {
    LOG(ERROR) << "Callback with given key not found";
    return false;
  }
...
bool RemoteCallbackMap<K, V>::UnregisterInternal(
  ...
  LOG(ERROR) << "Callback successfully removed from map";
...
void RemoteCallbackMap<K, V>::CallbackDeathRecipient::binderDied(
  ...
  LOG(ERROR) << "Callback from dead process unregistered; notifying delegate";
