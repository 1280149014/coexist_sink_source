...
LowEnergyAdvertiser::~LowEnergyAdvertiser() {
  ...
  LOG(ERROR) << "LowEnergyAdvertiser unregistering advertiser: " << advertiser_id_;
...
void LowEnergyAdvertiser::EnableCallback(bool enable, uint8_t advertiser_id,
  ...
  LOG(ERROR) << __func__ << "advertiser_id: " << advertiser_id
  ...
  if (enable) {
    ...
    if (status == BT_STATUS_SUCCESS) {
      LOG(ERROR) << "Multi-advertising stopped for advertiser_id: "
              << advertiser_id;
      adv_started_ = false;
    } else {
...
bool LowEnergyAdvertiserFactory::RegisterInstance(
  ...
  LOG(ERROR) << __func__ << " calling register!";
  ...
  LOG(ERROR) << __func__ << " call finished!";
...
void LowEnergyAdvertiserFactory::RegisterAdvertiserCallback(
  ...
  if (iter == pending_calls_.end()) {
    LOG(ERROR) << "Ignoring callback for unknown app_id: " << app_uuid.ToString();
    return;
  }
