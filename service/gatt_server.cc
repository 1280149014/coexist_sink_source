...
GattServer::~GattServer() {
  ...
  LOG(ERROR) << "GattServer unregistering: " << server_id_;
...
bool GattServer::AddService(const bluetooth::Service& service,
  ...
  LOG(ERROR) << __func__ << " server_id: " << server_id_;
  lock_guard<mutex> lock(mutex_);
...
bool GattServer::SendNotification(const std::string& device_address,
  ...
  LOG(ERROR) << " - server_id: " << server_id_
...
void GattServer::ConnectionCallback(
  ...
  LOG(ERROR) << __func__ << " conn_id: " << conn_id << " connected: " << connected
          << " BD_ADDR: " << device_address;

  if (!connected) {
    // Erase the entry if we were connected to it.
    LOG(ERROR) << "No longer connected: " << device_address;
...
void GattServer::ServiceAddedCallback(hal::BluetoothGattInterface* gatt_iface,
  ...
  LOG(ERROR) << __func__ << " - status: " << status << " server_id: " << server_id
  ...
  for (size_t i = 1; i < svc.size(); i++) {
    ...
    LOG(ERROR) << " - processing item no: " << i
...
void GattServer::RequestReadCharacteristicCallback(
  ...
  LOG(ERROR) << __func__ << " - conn_id: " << conn_id << " trans_id: " << trans_id
          << " BD_ADDR: " << device_address
...
std::shared_ptr<GattServer::Connection> GattServer::GetConnection(
  ...
  if (iter == conn_id_map_.end()) {
    LOG(ERROR) << "Connection doesn't belong to this server";
    return nullptr;
  }
  ...
  if (conn->request_id_to_handle.find(request_id) !=
    ...
    LOG(ERROR) << "Request with ID: " << request_id << " already exists for "
            << " connection: " << conn_id;
    return nullptr;
  }
...
bool GattServerFactory::RegisterInstance(const Uuid& uuid,
  ...
  LOG(ERROR) << __func__ << " - Uuid: " << uuid.ToString();
...
void GattServerFactory::RegisterServerCallback(
  ...
  if (iter == pending_calls_.end()) {
    LOG(ERROR) << "Ignoring callback for unknown app_id: " << uuid.ToString();
    return;
  }
